...
for i, (col, p_key) in enumerate(zip([c1, c2], ["p1", "p2"])):
    ...
    with col:
        st.markdown(f'<div class="player-box {"active-player" if is_active else ""}"><h3>PLAYER {i+1} {"(TURN)" if is_active else ""}</h3><h2 style="color: #00ff00;">HP: {p["hp"]}</h2><p>⚔️ Bonus: +{p["bonus"]} | ⚠️ Status: {", ".join([f"{k}({v})" for k,v in p["statuses"].items() if v>0]) or "Normal"}</p></div>', unsafe_allow_html=True)
    ...
    if len(p_now["hand"]) < 5:
        if cols[0].button("🎴 カードをドローして終了", use_container_width=True):
            if st.session_state.deck:
                ...
                switch_player()
                st.rerun()
    if cols[1].button("⚔️ 攻撃フェーズへ（ダイス確定）", use_container_width=True):
    ...
    if not available:
        ...
        if st.button("🔥 発動！", use_container_width=True):
            ...
            if not found:
                ...
                if len(p_now["used_innate"]) == 3: p_now["used_innate"] = []; p_now["bonus"] += 10; add_log("🌟 覚醒！")
